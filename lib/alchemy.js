	...
	function iterateEffects(ingredients, iterator) {
		...
		try {
			for (var _iterator = ingredients[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
				...
				try {
					for (var _iterator2 = ingredient.effects[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
						var _step2$value = _slicedToArray(_step2.value, 2);

						var effect = _step2$value[0];
   ...
   */
     ...
     */
				...
				value: function hasSomeEffects(effects) {
					...
					return effects.some(function (eff) {
						return _this.hasEffect(eff);
					});
				},
				writable: true,
				configurable: true
	...
	function getSuitableIngredients(givenIngredient, ingredients) {
		...
		iterateEffects(ingredients, function (effect, ingredient) {
			if (ingredient.name !== givenIngredient.name) {
